...
bool DependencyAnalyzer::checkCommand(int argc, char* argv[]) {
	if (argc < 2)
	{
	...
	try {
		subdir = FileSystem::Path::getFullFileSpec(argv[1]);
		...
		{
			searchPattern.push_back(argv[i]);
		}
...
// return ComFilePath
std::string DependencyAnalyzer::getComFilePath() {
	return ComFilePath;
}
