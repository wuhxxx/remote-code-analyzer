...
void TypeAnalyzer::parsePubInfo(CodeAnalysis::ASTNode* pgs) {
	try {
		for (CodeAnalysis::ASTNode* an : pgs->children_) {
			if (an->type_ == "class" || an->type_ == "function") {
				//an->type_ == "class" || an->type_ == "struct" || an->type_ == "function"
			...
			if (an->type_ == "class" || an->type_ == "namespace") { //an->type_ == "class" || an->type_ == "struct"
				parsePubInfo(an);
			}
		}
	}
