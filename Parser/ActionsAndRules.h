...
{  
  ...
  public:
    bool doTest(const Scanner::ITokCollection* pTc) override
    {
      ...
      const Scanner::ITokCollection& tc = *pTc;
	  // to avoid typedef class, add second condition
	  size_t let = tc.find("typedef");
      if (tc[tc.length() - 1] == "{" && let == tc.length())
      {
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (operIndex < pTc->length()) {
        ...
        if ((*pTc)[operIndex + 2] != "(") { name += (*pTc)[operIndex + 2]; }
        top->name_ = name;
        nameIndex = operIndex;
      }
      ...
      if (posOpenBracket < posCloseBracket && posBrace == (posCloseBracket + 1) && posBrace < pTc->length()) {
        std::string name;
        for (size_t i = posOpenBracket; i < posBrace; ++i) { name += (*pTc)[i]; }
        top->name_ = name;
        top->type_ = "lambda";
      }
    }
  ...
  */
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
	  ...
	  // add typedef declaration identification on Mar.04, 2017
	  if (se[0] == "typedef")
	  {
  ...
  */
  ...
  // rule to detect enum statements, added by Hongxin WU on Mar.03, 2017
  ...
  // action to add enum info to scope stack top, added by Hongxin WU on Mar.03, 2017
